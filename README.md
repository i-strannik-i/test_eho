...
Или через готовый Windows-ярлык:
...
- проверит `Ollama`
- подтянет модель `qwen2.5:3b`, если её нет
- и только потом запустит GUI
...
Этот сценарий нужен для переезда на другой ПК. Он:
- при необходимости поднимет запуск от имени администратора
- установит `Python`, если его нет
- установит `Ollama`, если её нет
- создаст локальное окружение `.venv`
- подготовит базовый runtime для `GUI + Ollama`
- проверит импорты GUI
- проверит наличие модели `qwen2.5:3b`

После этого можно запускать `start_echo.bat`.

### Расширенный training stack

Если на новом ПК нужен именно стек для локального дообучения, датасетов и Hugging Face:

```bat
setup_training_stack.bat
```

Это отдельный шаг, потому что он тянет тяжёлые ML-зависимости и сильнее зависит от сети, индекса пакетов и доступности wheel-файлов.
